...
<script>
/* api */
import * as theApi from '@api/dataManagement/documentRecycleBin';
/* 组件 */
...
export default {
	name: 'data-management-document-recycle-bin',
	data() {
		return {
			/* 表格配置 */
			configData: {
	...
	methods: {
		...
		completelyDelete: function(row = {}) {
			const { id } = row;
			this.$confirm('是否要彻底删除此文档（无法恢复）?', '提示', {
				confirmButtonText: '确定',
				cancelButtonText: '取消',
				type: 'warning'
			})
